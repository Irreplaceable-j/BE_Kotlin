...
class ExtensionKtTest{
    ...
    @Test
    fun testMap(){
        val names = listOf("유은영", "조희진", null, "민다빈")
        val listOfList = listOf(mutableListOf(1,2,3,4), mutableListOf(3,4,5))

        println("null이 아닌 문자열의 길이 ${names.mapNotNull { it?.length }}")
        println("flatten map ${listOfList.flatten()}")
        println("flat map ${listOfList.flatMap { 
            it.addLast(10000)
            it
        }}")
    }
